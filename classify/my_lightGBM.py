...
def tune_lgb(x_train_balanced, y_train_balanced, x_val_scaled, y_val_np, x_test_scaled, y_test_np, random_state,
             balancing_method, dataset_name, generation_time=0.0, use_gpu=True):
    import time
    """
    Main function for LightGBM tuning with optional GPU support.
    
    Parameters:
    -----------
    use_gpu : bool
        If True, uses GPU for training (requires CUDA and LightGBM compiled with GPU support)
    """
    ...
    # Pipeline
    pipes = {'lgb': Pipeline([('model', lgb)])}

    # Grids (reduced from 60 to ~18 parameter combinations)
    learning_rate_grids = [0.05, 0.1, 0.2]
    num_leaves_grids = [31, 63, 127]
    min_data_in_leaf_grids = [20, 50]

    param_grids = {}
    param_grids['lgb'] = [{'model__learning_rate': learning_rate_grids,
                           'model__num_leaves': num_leaves_grids,
    ...
    print(f"   -> Training LightGBM on {device_str}")
